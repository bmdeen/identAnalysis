...
function identTPROIExtract(defineROITask,contrastName,searchName,isCifti,varargin)

% Define constants
fppDir = '/path/to/data/derivatives/fpp';
...
if isCifti
    searchSpace = 'individual_den-32k'; % 'fsLR_den-32k' in identROIExtract
...
if ismember(contrastName,{'PersonVsPlace','BeliefVsPhoto','FacesVsScenes',...
        'SentencesVsNonwords','PersonVsObject'}) && invertStats==0
...
elseif ismember(contrastName,{'PlaceVsObject','PersonVsObject','FacesVsObjects',...
    ...
    %statsToPlot = 19:28;   % From paralell systems paper script
    statsToPlot = [3 6 19:28];
elseif ismember(contrastName,{'AllVsRest'})
    statsToPlot = 1:28;
elseif ismember(contrastName,{'ObjectVsPersonPlace'})
    statsToPlot = [3 6 19:28];
end
...
%     end
    ...
    %%% HACK for AllVsRest contrasts run using modelperm
    if strcmp(contrastName,'AllVsRest')
        defineROIDir = fpp.bids.changeName([analysisDir '/sub-' subject '_task-'...
            defineROITask '_run-01_space-' statSpace '_modelperm'],'desc',inputSuffix);
    end
    
    for t=1:length(tasks)
        extractResponseDir = fpp.bids.changeName([analysisDir '/sub-' subject '_task-' tasks{t} ...
            '_run-01_space-' statSpace '_modelarma'],'desc',strrep(inputSuffix,'AllVsRest',''));
