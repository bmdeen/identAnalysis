function varargout=identTPSigStarSmallPlot(groups,stats,nosort)
    ...
    %
    % Edited by Ben Deen, 04/2021 (identSigStar):
    % - Changed p-value cutoffs
    % - Added defaultFontSize constant.
    %
    % Edited by Ben Deen, 08/2021 (identSigStarSmallPlot):
    % - Increased distance between sig lines, and reduced font size, for
    %   smaller (550 x 140) bar plots. Changed highest p cutof from .01 to
    %   .0071 - intended for ROI analysis, Bonferroni-corrected .05 for 7
    %   ROIs.
    ...
    y=ylim;
    yd=myRange(y)*0.10; %separate sig bars vertically by 10% 
    ...
    %for all bars.
    yd=myRange(ylim)*0.02; %Ticks are 2% of the y axis range
    for ii=1:length(groups)
        y=get(H(ii,1),'YData');
        y(1)=y(1)-yd;
        y(4)=y(4)-yd;   
...
function H=makeSignificanceBar(x,y,p)
    ...
    H(2)=text(mean(x(:)),starY,stars,...
        ...
        'Tag','sigstar_stars','FontSize',defaultFontSize);
